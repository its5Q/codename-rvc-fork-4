...
import rvc.lib.algorithm.generators.wavehax_modules as wavehax_modules #
from rvc.lib.algorithm.generators.wavehax_modules import (
    STFT,
    ComplexConv1d,
    ComplexConv2d,
...
class WavehaxGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.prior_generator = partial(
            getattr(wavehax_modules, f"generate_{prior_type}"),
        ...
        if gin_channels != 0:
            # gin_channels (256) to in_channels (192) proj
            self.cond_g = nn.Conv1d(gin_channels, in_channels, 1) 
    ...
    def forward(self, feat: Tensor, f0: Tensor, g: Optional[Tensor] = None) -> Tensor:
        """
        Calculate forward propagation.

        Args:
            feat (Tensor): Conditioning features with shape (batch, in_channels, frames).
            f0 (Tensor): F0 sequences with shape (batch, 1, frames).
            g (Tensor): Speaker embedding conditioning (batch, gin_channels, 1)
        ...
        feat = self.cond_proj(feat)

        # Convert to 2d representation
        x = torch.stack([prior1, prior2, prior1_proj, prior2_proj, feat], dim=1)
        x = self.input_proj(x)
        x = self.input_norm(x)
        ...
        # To avoid istft issues with fp16
        original_dtype = real.dtype
        if original_dtype == torch.float16:

            # Upcasting to fp32 to bypass the cuFFT power-of-two constraint ~
            real_fp32 = real.float()
            imag_fp32 = imag.float()

            # stft inversion
            x = self.stft.inverse(real_fp32, imag_fp32)
    ...
    def inference(self, feat: Tensor, f0: Tensor) -> Tensor:
        return self(feat, f0)[0]
